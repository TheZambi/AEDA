...
TEST(test_1, test_d) {
	...
	*/
	...
	EXPECT_EQ(1, sl1.size());
	EXPECT_EQ("t.silva@gmail.com", sl1[0].getEMail());

	organization.addStudent(&s2);
	vector<StudentPtr> sl2 = organization.getStudents();
	EXPECT_EQ(1, sl2.size());
...
TEST(test_1, test_e) {
	...
	*/
	...
	sl1 = o1.getStudents();
	EXPECT_EQ("tiago.sousa@gmail.com", sl1[0].getEMail());
	...
	for(unsigned i = 0; i < sl3.size(); i++) {
		if(sl3[i].getName() == "Ana") EXPECT_EQ("a.gomes@gmail.com", sl3[i].getEMail());
	}
...
TEST(test_1, test_f) {
	...
	*/
	...
	EXPECT_EQ(1, o1.getActiveStudents().size());

	FEUPConsulting o2;
	o2.addActiveStudents(candidates_0, 2);
	EXPECT_EQ(0, o2.getActiveStudents().size());

	FEUPConsulting o3;
	o3.addActiveStudents(candidates_5, 3);
	EXPECT_EQ(2, o3.getActiveStudents().size());
	EXPECT_EQ("Pedro",o3.getActiveStudents().top().getName());
	...
	EXPECT_EQ(1, o4.getActiveStudents().size());
	...
	EXPECT_EQ(1, o5.getActiveStudents().size());
...
TEST(test_1, test_g) {
	...
	*/
	...
	EXPECT_EQ(0, o1.mostActiveStudent(maximus));
	...
	EXPECT_EQ(0, o2.mostActiveStudent(maximus));
	EXPECT_EQ(3, o2.getActiveStudents().size());
	...
	EXPECT_EQ(3, o3.mostActiveStudent(maximus));
	EXPECT_EQ("Pedro", maximus.getName());
	EXPECT_EQ(3, o3.getActiveStudents().size());
	...
	EXPECT_EQ(2, o4.mostActiveStudent(maximus));
	EXPECT_EQ("Maria", maximus.getName());
	EXPECT_EQ(2, o4.getActiveStudents().size());
